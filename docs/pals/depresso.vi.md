---
entity: pal
paldex: 16
name: Depresso
slug: depresso
code: NegativeKoala
element: [dark]
rarity: 1
size: xs
food_consumption: 1
work_suitability:
  handiwork: 1
  mining: 1
  transporting: 1
  farming: 1
partner_skill:
  name: Caffeine Inoculation
  effect: When activated, Depresso chugs an entire energy drink, increasing its own Movement Speed and Work Speed by 100–900%. Sometimes drops Venom Gland when assigned to a Ranch.
stats:
  health: 70
  food: 100
  melee_attack: 70
  attack: 70
  defense: 70
  work_speed: 100
  support: 100
capture_rate_correct: 1
male_probability: 50
egg: Dark Egg
gold_coin: 436
level_80:
  health: [3700, 4540]
  attack: [520, 646]
  defense: [470, 596]
drops:
  - item: [[venom-gland]]
    qty: "2-3"
    chance: 100%
breeding:
  rank: 3000
  known_combos: []
spawns: []
tags: [pal, dark]
---

# Depresso <small>#16</small>

> Nó ít bạn vì gương mặt lúc nào cũng cau có, nhưng thực ra lại tốt bụng. Có người
> từng thấy nó cho [[vixy|Vixy]] lạc bầy ăn.

Một Pal hệ [[elements|Dark]] thời kỳ đầu — Paldeck #16, cỡ XS, độ hiếm 1. Một thợ
căn cứ đa dụng có thể tự tăng vọt tốc độ làm việc của mình.

## Thức ăn

<span class="food-meter" data-value="1" data-max="10"></span>

Mức ăn **1 / 10** — rất thấp, nuôi cực rẻ.

## Partner skill

**Caffeine Inoculation** — khi kích hoạt, Depresso tu cạn một lon nước tăng lực,
tự tăng **Tốc độ di chuyển và Tốc độ làm việc của chính nó thêm 100–900%** (chỉ
bản thân). Nó cũng **thỉnh thoảng cho [[venom-gland|Tuyến Nọc Độc]]** khi cắm
vào [[ranch|Trang trại]]. Mức tăng Work Speed cho bản thân theo cấp:

| Lv | Work Speed + (bản thân) |
|:--:|:-------------------:|
| 1 | +100% |
| 2 | +200% |
| 3 | +400% |
| 4 | +600% |
| 5 | +900% |

Sản lượng [[venom-gland|Tuyến Nọc Độc]] ở Trang trại theo cấp (mỗi lần đều chắc chắn):

| Lv | Tuyến Nọc Độc | Tỉ lệ |
|:--:|-------------|:------:|
| 1 | ×1 | 100% |
| 2 | ×1–2 | 100% |
| 3 | ×1–3 | 100% |
| 4 | ×1–4 | 100% |
| 5 | ×1–5 | 100% |
| 6 | ×1–6 | 100% |
| 7 | ×1–7 | 100% |
| 8 | ×1–8 | 100% |
| 9 | ×2–9 | 100% |
| 10 | ×3–10 | 100% |

## Công việc & vai trò ở căn cứ

|  | Công việc | Cấp độ |
|:----:|------|:--:|
| ![](../assets/icons/work/handiwork.png){ .game-icon } | [Handiwork](../mechanics/work/handiwork.md) | 1 |
| ![](../assets/icons/work/mining.png){ .game-icon } | [Mining](../mechanics/work/mining.md) | 1 |
| ![](../assets/icons/work/transporting.png){ .game-icon } | [Transporting](../mechanics/work/transporting.md) | 1 |
| ![](../assets/icons/work/farming.png){ .game-icon } | [Farming](../mechanics/work/farming.md) | 1 |

Bốn việc đều cấp 1 — thợ căn cứ linh hoạt, và buff tự tăng tốc làm việc khiến nó
làm hiệu quả bất ngờ.

## Chiến đấu

Hệ [[elements|Dark]] — mạnh trước Neutral, yếu trước Dragon. Chỉ số đều thấp
(Health/Attack/Defense 70); không phải Pal đánh nhau. Ở cấp 80 đạt Health
3700–4540, Attack 520–646, Defense 470–596.

## Nhân giống

CombiRank 3000. Nở từ **Dark Egg**. Chưa ghi nhận cặp bố mẹ nào. Xem [[breeding]].

## Vật phẩm rơi

Khi bắt hoặc hạ gục:

|  | Vật phẩm | SL | Tỉ lệ |
|:----:|------|:---:|:------:|
| ![](../assets/icons/items/venom-gland.png){ .game-icon } | [Tuyến Nọc Độc](../items/materials/venom-gland.md) | ×2–3 | 100% |

## Nơi tìm thấy

Chưa ghi nhận vị trí xuất hiện.
