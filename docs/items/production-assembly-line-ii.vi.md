---
entity: item
name: Production Assembly Line II
slug: production-assembly-line-ii
category: structure
code: Factory_Hard_03
build:
  category: Production
  tech_level: 42
  build_work: 50000
  ingredients:
    - item: [[refined-ingot]]
      qty: 100
    - item: [[circuit-board]]
      qty: 10
    - item: [[cement]]
      qty: 30
    - item: [[corrosive-solvent]]
      qty: 5
operate_suitability: handiwork 1
energy: 400 per sec
energy_type: Electric
sanity: -0.2
defense: 4
tags: [item, structure]
---

# Dây chuyền sản xuất II (Production Assembly Line II)

> Dây chuyền cao cấp nhất — quản lý chặt chẽ đẩy tốc độ sản xuất vượt xa dây
> chuyền đời đầu. Ngốn điện lớn.

Trạm [[handiwork|Handiwork]] mạnh nhất game, bản nâng cấp của
[[production-assembly-line|Dây chuyền sản xuất]]. Nó tiêu điện gấp đôi (400
điện/giây) để đạt sản lượng cao nhất. Hao tinh thần nhiều hơn (SAN −0.2); phòng
thủ 4.

## Chế tạo tiêu biểu

Sản xuất [[gunpowder|Thuốc súng]] và các vật phẩm bậc cao nhất ở tốc độ tối đa.

## Xây dựng

Mở tại **Công nghệ Cấp 42**. Công sức xây: 50000 ([[handiwork|Handiwork]]).
Cần nguồn điện để hoạt động.

|  | Vật liệu | SL |
|:--:|----------|:--:|
| ![](../assets/icons/items/refined-ingot.png){ .game-icon } | [Refined Ingot](refined-ingot.md) | 100 |
| ![](../assets/icons/items/circuit-board.png){ .game-icon } | [Circuit Board](circuit-board.md) | 10 |
| ![](../assets/icons/items/cement.png){ .game-icon } | [Xi Măng](cement.md) | 30 |
| ![](../assets/icons/items/corrosive-solvent.png){ .game-icon } | [Corrosive Solvent](corrosive-solvent.md) | 5 |
